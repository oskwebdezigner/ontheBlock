...
import { Picker, Image, StyleSheet, Text, TouchableOpacity, View , 
  Platform , ActionSheetIOS , Modal ,TouchableWithoutFeedback ,Dimensions , Alert, ActivityIndicator} from 'react-native';
...
function CameraComponent(props) {
    ...
    const openCameraPickerAsync = async () =>{
      ...
      let cameraResult = await ImagePicker.launchCameraAsync({
        ...
        let imageUri = cameraResult ? `data:image/jpg;base64,${cameraResult.base64}` : null;
        let img = {
            base64_image : imageUri,
        }
        props.update(imageUri)
      ...
      return (
        <View style={styles.container}>
            <TouchableOpacity onPress={closeSelectedImage}>
                <FontAwesome name="close" size={24} color={currentTheme.themeBackground} />
            </TouchableOpacity>
          <Image
   ...
   {Loader ? <ActivityIndicator size={24} color={'orange'} />  :
       <TouchableOpacity style={{margin : 5}} onPress={() => 
            Platform.OS === 'ios' ? onPress() :
  ...
  <View style={[styles.centeredView,{backgroundColor: 'rgba(0, 0, 0, 0.7)',justifyContent :"flex-end"}]}>
      <View style={styles.modalView}>
    
     <TouchableOpacity 
       onPress={() => {IOSPicker()}}
     style={[styles.appButtonContainer,{justifyContent :'center',margin : 5,  borderBottomWidth : 0.6,
     ...
     style={[styles.appButtonContainer,{justifyContent :'center',margin : 5,  
    ...
    //  borderBottomColor : currentTheme.themeBackground
      ...
      borderRadius: 10 ,marginRight : 20,marginLeft : 20,marginBottom : 20}}>
          <TouchableOpacity style={[styles.CheckBoxBtn,{margin : 5 ,justifyContent :'center'}]}
